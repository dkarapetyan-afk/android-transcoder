...
- Captions on Compress: first use downloads Whisper tiny (~100 MB). A clip with speech gets a togglable subtitle track in VLC plus an `.srt` next to the gallery file. Mute skips captions. A captions failure still publishes the video. A second **Transcribing** notification shows percent and phase (Downloading speech model / Extracting audio / Transcribing / Muxing captions). Cancel on that notification skips captions and still publishes the video. Cancel on Compressing video still cancels the job. Device engine + WebM (Opus) must still caption; Result → View log should show `pcm=mediacodec` and `muxed subtitles` without FFmpeg Opus “Extradata version 19” spam. Old Media3 WebMs may log `opus CodecPrivate rewritten for FFmpeg` once.
- Captions on Record (direct encode, mic or internal audio): after stop, same subtitle track + `.srt`. Without audio, the switch does nothing at stop. The Transcribing notification appears beside Saving recording; Cancel there skips captions and still saves the file.
