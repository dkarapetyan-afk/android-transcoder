...
- Named compression profiles: save the current Compress options and load them later from a dropdown
- Optional on-device captions: Whisper tiny via sherpa-onnx, muxed as a subtitle track plus an `.srt` sidecar. Optional burn-in paints each cue onto the frames at its Whisper timestamp (always visible; re-encodes the picture). The speech model downloads once (~100 MB) and audio never leaves the phone. A Transcribing notification shows progress; Cancel there skips captions and keeps the file.
